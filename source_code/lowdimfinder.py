...
class LowDimFinder(object):
    ...
    def _define_groups(self):
        ...
        threshold_allowance = self.params["bond_margin"]

        max_nearest_distance = get_max_nearest_neighbor_distance(self.aiida_structure)
        pairs = get_bonded_pairs(coords, max_nearest_distance, threshold_allowance , use_scipy=True)
    ...
    def _define_number_of_connections(self):
        ...
        tuple_list = [(counter, idx) for idx, counter in counter_dict.iteritems()]
        ...
        for i in range(self.supercell_size**3):
            if i*self.n_unit + periodic_site_in_unit_cell in group:
                ...
                unpos.append(self.supercell.get_positions()[i*self.n_unit+periodic_site_in_unit_cell])
	...
	for i in self._get_unconnected_positions()[self._low_dim_index]:
	    unconnected_vectors.append(i- self._connected_positions[self._low_dim_index][0])
	...
	if self._dimensionality[self._low_dim_index] == 0:
		...
		while np.linalg.norm(np.dot(np.cross(vector1,vector2),vector3)) < 0.0000001:
		    idx = find_shortest_vector(unconnected_vectors)
		    vector3 = unconnected_vectors.pop(idx)
	    ...
	    if ( l_are_equals(norm_vector1,norm_vector2) 
		 and a_are_equals(cosphi,0.5) ):
	...
	if self._get_dimensionality()[self._low_dim_index] == 3:
		...
		# min and max z
		normal_vec_z = np.cross(self._vectors[self._low_dim_index][0],self._vectors[self._low_dim_index][1])
		current_z = np.dot(positions[-1],normal_vec_z/np.linalg.norm(normal_vec_z))
		...
		normal_vec = np.cross(self._vectors[self._low_dim_index][0],self._vectors[self._low_dim_index][1])

		# cosinus between third vector and normal vector
		cos = abs(np.dot(normal_vec, self._vectors[self._low_dim_index][2])/np.linalg.norm(normal_vec)/np.linalg.norm(self._vectors[self._low_dim_index][2]))

		# translate the lengths into direction of the third vector
		vacuum_space_in_third_direction = float(self.params["vacuum_space"])/cos
		max_z = max_z / cos
		min_z = min_z / cos
			
		positions = positions - (min_z - vacuum_space_in_third_direction/2) * self._vectors[self._low_dim_index][2]
		self._vectors[self._low_dim_index][2] = self._vectors[self._low_dim_index][2] * (vacuum_space_in_third_direction + max_z-min_z)
	...
	reduced_ase_structure = Atoms(cell=self._vectors[self._low_dim_index],
			    pbc=pbc, symbols=symbols, positions=positions, tags=tags)
	...
	if self.params['orthogonal_axis_2D']:
	    originalstruc.rotate(v=asestruc.cell[2],a=[0,0,1],center=(0,0,0), rotate_cell = True)
	    ...
	    originalstruc.rotate(v=normal_vec,a=[0,0,1],center=(0,0,0), rotate_cell = True)
	    asestruc.rotate(v=normal_vec,a=[0,0,1],center=(0,0,0), rotate_cell = True)
	    # turn it into positive z direction (if needed)
	    if asestruc.cell[2][2] < 0.:
		originalstruc.rotate(v=[0,0,np.sign(asestruc.cell[2][2])],a=[0,0,1],center=(0,0,0), rotate_cell = True)
		asestruc.rotate(v=[0,0,np.sign(asestruc.cell[2][2])],a=[0,0,1],center=(0,0,0), rotate_cell = True)
	...
	originalstruc.rotate(v=asestruc.cell[0],a=[1,0,0],center=(0,0,0), rotate_cell = True)
	...
	for asestruc in self._rotated_structures:
	    rotated_aiida_structures.append(S(ase=asestruc))
	...
	for idx, dim in enumerate(self._dimensionality):
		...
		asestruc = Atoms(cell=[self._vectors[idx][0],self._vectors[idx][1],self._shortest_unconnected_vector[counter]],
			positions=self.structure.get_positions(),
			symbols=self.structure.get_chemical_symbols(), 
			tags=self.structure.get_tags(),pbc=[True,True,True])
		...
		if int(round(asestruc.get_volume()/self.structure.get_volume()))!=1:
		    ...
		    the_asestruc = Atoms(cell=[self._vectors[idx][0],self._vectors[idx][1],self._shortest_unconnected_vector[counter]],
			positions=self.supercell.get_positions(),symbols= self.supercell.get_chemical_symbols(), pbc=[True,True,True])
		    
		    pos_and_symbols = zip(the_asestruc.get_scaled_positions().tolist(),
					  the_asestruc.get_chemical_symbols())
		    the_pos,the_symbols = zip(*objects_set(pos_and_symbols))
		    asestruc = Atoms(cell=[self._vectors[idx][0],self._vectors[idx][1],self._shortest_unconnected_vector[counter]],
			scaled_positions=the_pos,symbols=the_symbols,pbc=[True]*3)
...
def get_max_nearest_neighbor_distance(aiida_structure):

    """
    --------------------------------------------------------------------------------    
    |RDF calculation                                                               |
    |For 1 atom over its neighbors with 1 frame                                    |
    |                   p(r)                                                       |
    | g(r) =  lim  -----------------                                               |
    |        dr->0  4Pi (N/V) r^2dr                                                |
    |                                                                              |
    | where                                                                        |
    |       1. p(r):     number of atom pairs at distance between r+dr and r       |
    |       2. N=N_pair: number of unique pairs of atoms from two sets, set1 & set2|
    --------------------------------------------------------------------------------

    This function is trying to identify the maximun nearest neighber of an input structure.
    STEPS:
        1. Spot maximum of the nearest neighbor distances (Dmax) of every elemants in the structure.
	2. Spot all the pair(A,B) with distance equals Dmax and denote.
        3. Run the RDF for A in pari(A,B) with respect to element type B to avoid missing the potentially 'bigger' D, which may be slightly bigger.
        4. Spot the MAX peaks distance in RDF whithin first peak + 0.1 Angstromg and return it.
    ...
    position_array = ase_structure.get_positions()
    cell = ase_structure.get_cell()
    distances_matrix = ase.geometry.get_distances(p1=position_array,p2=None,cell=cell,pbc=(1,1,1))[0]
    ...
    for i in range(len(ase_structure)):
        min_distance = 10.
        for j in range(len(ase_structure)):
            distance = np.linalg.norm(distances_matrix[i][j])
            if distance !=0 and distance < min_distance:
                min_distance = distance
        nearest_distances_list.append(min_distance)

    max_nearest_distance = max(nearest_distances_list)
    for i in range(len(ase_structure)):
        for j in range(len(ase_structure)):
            if np.linalg.norm(distances_matrix[i][j]) == max_nearest_distance:
                crucial_atom_index.append([i,j])
    ...
    for i in crucial_atom_index:
        type_1, type_2 = zip(i)
	for j in range(len(ase_structure)):
	    if ase_structure.get_atomic_numbers()[j] == ase_structure.get_atomic_numbers()[type_2]:
		if type_1 != j:
    		    distances_list.append(np.linalg.norm(distances_matrix[type_1][j]))

        count, radius, patch = plt.hist(distances_list, bin_number, range=(1,max_box_len))
        rdf = [ count[j] / (4*math.pi*radius[j]**2*(max(radius)-min(radius))/bin_number) for j in range(len(count)) ]
	plt.clf()

	first_peak = 0
	for k in range(len(rdf)):
	    n = k + 1
	    if rdf[n]>rdf[n-1] and rdf[n]>rdf[n+1]:
		if first_peak == 0: first_peak = radius[n]; break
	
	for l in range(len(rdf)):
	    if rdf[l] != 0 and radius[l] - first_peak <= 0.1 and radius[l] - first_peak >= 0.:
		neighbor_list.append(radius[l])

    return max(neighbor_list)

def get_bonded_pairs(coords, distance_threshold, allowance=0., use_scipy=True):
    """
	...
	try:
	    ...
	    raise ValueError("To use scipy and speed-up the lowdimfinder, you "
			     "need scipy with a version >= 0.16.0")

	# prefiltering the pairs by twice the maximum radius
	pairs = list(tree.query_pairs(2.* distance_threshold))
	...
	if len(pairs) > 0:
		...
		((coords1 - coords2)**2).sum(axis=1) - (allowance + distance_threshold)**2 < 0.)
	...
	pairs = zip(*np.nonzero(dist_squared - (factor * distance_threshold)**2 < 0))
